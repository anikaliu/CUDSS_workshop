...
Now we would like to generate the same plot, but for all of the measured circumfences in the dataset. One useful way fur this is so-called facetting. This requres some reshaping of the data:
...
```
Melt is using the columns you are listing to create a unique ID, then it stacks the remaining columns in a single column (more info: https://i1.wp.com/www.studytrails.com/wp-content/uploads/2016/09/reshape.png).
...
plot_scatter <- ggplot(body_fat_melt, aes(valßue, percent.fat))+
                      geom_point() +
                      geom_smooth(method= "lm") 
...
```
Now it's your turn: Do the derived correlation values agree with your expectations form the plot?ß
